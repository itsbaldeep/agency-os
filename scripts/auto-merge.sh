...
for REPO in agency-os agency-dashboard; do
  gh pr list --repo "$REPO" --json number,title,labels,createdAt,statusCheckRollup,author \
    -q '.[] | [.number, (.labels|map(.name)|join(",")), .createdAt, .author.login, .title] | @tsv' \
  | while IFS=$'\t' read -r N LABELS CREATED AUTHOR TITLE; do
      ...
      ALLOK=$(gh pr view "$REPO" "$N" --json statusCheckRollup \
      ...
      gh pr merge "$REPO" "$N" --merge --delete-branch --admin && echo "merged $REPO PR#$N: $TITLE" \
